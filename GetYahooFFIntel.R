...
creds <- read.csv(paste0(getwd(),"/YahooAPI.csv"), stringsAsFactors=F)
...
responseAllTeams<- read_html(paste0(league.url, league.key, "/teams"), config(token = token))
#responseAllTeams %>% xml_structure()
Name <- responseAllTeams %>% html_nodes('name')

responseAllPlayersOnRoster<- read_html(paste0(team.url, my.team.key, "/roster/players"), config(token = token))
...
responseAllPlayersFree <- read_html("http://fantasysports.yahooapis.com/fantasy/v2/league/348.l.982118/players?&status=A&start=0&stat1=S_PW_1",config(token=token))
responseAllPlayersFree<- read_html(url, config(token = token))
...
yahooHTML<-lapply(YahooURLs,function(x) {Sys.sleep(abs(rnorm(1)*4+7));read_html(x, config(token = token))})
